...
全屏任务开始后，AEye 会自动最小化，避免自身窗口遮挡目标程序或接收到模型点击；任务结束或失败后自动恢复。为降低首轮和后续规划延迟，主模型接收缩小后的规划截图，并且每轮只携带当前截图、只保留界面配置的最近几轮文字轨迹，避免截图和完整计划随步骤无限累积；标准模式下 UI-TARS 接收原始全屏坐标截图，输出直接对应虚拟桌面物理坐标。两张图都由同一时刻的同一次桌面截图生成，因此不会产生界面状态错位。主模型单次请求超过 30 秒会快速失败并进入安全回退，避免界面长期卡死。快速模式不调用独立 Grounding 模型。Windows 下 `open(name)` 会优先激活已经运行的匹配窗口，未运行时通过剪贴板执行支持中文的开始菜单搜索；对微信等已识别应用的开始菜单或任务栏图标点击也会自动转换为窗口激活，避免视觉误认相邻图标。`switch_applications(name)` 只激活已经打开的匹配窗口，不会启动第二个应用实例。
