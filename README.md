...
全屏任务开始后，AEye 会自动最小化，避免自身窗口遮挡目标程序或接收到模型点击；任务结束或失败后自动恢复。为降低首轮和后续规划延迟，主模型接收缩小后的规划截图，并且每轮只携带当前截图、保留历史文字轨迹，避免截图随步骤累积；标准模式下 UI-TARS 接收原始全屏坐标截图，输出直接对应虚拟桌面物理坐标。两张图都由同一时刻的同一次桌面截图生成，因此不会产生界面状态错位。主模型单次请求超过 30 秒会快速失败并进入安全回退，避免界面长期卡死。快速模式不调用独立 Grounding 模型。`switch_applications(name)` 只激活已经打开的匹配窗口，不会启动第二个应用实例。
