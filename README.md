...
| 全屏多窗口 | 按桌面原始分辨率截取完整虚拟桌面，允许 `Alt+Tab`、任务栏或应用动作切换窗口 | 需要在多个已打开程序之间复制、核对或连续操作 |
...
全屏任务开始后，AEye 会自动最小化，避免自身窗口遮挡目标程序或接收到模型点击；任务结束或失败后自动恢复。全屏模式中的 UI-TARS 坐标直接对应虚拟桌面物理坐标，不再经过截图缩放。`switch_applications(name)` 只激活已经打开的匹配窗口，不会启动第二个应用实例。
